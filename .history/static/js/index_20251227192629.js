...
document.addEventListener('DOMContentLoaded', function() {
    ...
    navLinks.forEach(link => {
        ...
        if (currentPath.includes(linkPath.replace('/', '')) || 
            ...
            this.style.color = 'var(--cinema-gold)';
        }
    });
    ...
    // 10. Botón de modo cine (opcional)
    const cinemaModeBtn = document.createElement('button');
    cinemaModeBtn.innerHTML = '🎬 Modo Cine';
    cinemaModeBtn.style.cssText = `
        position: fixed;
        bottom: 20px;
        right: 20px;
    ...
    cinemaModeBtn.addEventListener('mouseenter', function() {
        this.style.backgroundColor = 'var(--cinema-accent)';
        this.style.transform = 'scale(1.1)';
    });
    
    cinemaModeBtn.addEventListener('mouseleave', function() {
        this.style.backgroundColor = 'var(--cinema-primary)';
        this.style.transform = 'scale(1)';
    });
    
    cinemaModeBtn.addEventListener('click', function() {
        document.body.classList.toggle('cinema-mode');
        
        if (document.body.classList.contains('cinema-mode')) {
            this.innerHTML = '📱 Modo Normal';
            document.body.style.overflow = 'hidden';
            showNotification('Modo Cine activado');
        } else {
            this.innerHTML = '🎬 Modo Cine';
            document.body.style.overflow = 'auto';
            showNotification('Modo Normal activado');
        }
    });
    
    document.body.appendChild(cinemaModeBtn);
    
    console.log('✅ Efectos de cine cargados correctamente');
});

// Funciones globales para otras páginas
window.CinemaEffects = {
    showNotification: function(message) {
        // Implementación de notificación para uso global
        console.log('Cinema Notification:', message);
    },
